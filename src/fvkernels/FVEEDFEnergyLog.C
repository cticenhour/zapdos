...
#include "FVEEDFEnergyLog.h"

registerMooseObject("ZapdosApp", FVEEDFEnergyLog);

InputParameters
FVEEDFEnergyLog::validParams()
{
...
FVEEDFEnergyLog::FVEEDFEnergyLog(const InputParameters & parameters)
...
ADReal
FVEEDFEnergyLog::computeQpResidual()
{
  return -_reaction_coefficient[_qp] * std::exp(_em[_qp] + _target[_qp]) *
         _threshold_energy;
}
