...
SakiyamaSecondaryElectronBC::computeQpResidual()
{
  ...
  return  -_test[_i][_qp] * _r_units * 2. / (1. + _r) * (1. - _a) * _user_se_coeff * _ion_flux *
             _normals[_qp];
}
...
SakiyamaSecondaryElectronBC::computeQpJacobian()
{
  ...
  _ion_flux = _sgnip[_qp] * _muip[_qp] * -_grad_potential[_qp] * _r_units * std::exp(_ip[_qp]) -
              _Dip[_qp] * std::exp(_ip[_qp]) * _grad_ip[_qp] * _r_units;
  // _n_gamma = (1. - _a) * _user_se_coeff * _ion_flux * _normals[_qp] / (_muem[_qp] *
  // -_grad_potential[_qp] * _r_units * _normals[_qp] + std::numeric_limits<double>::epsilon());
  _actual_mean_en = std::exp(_mean_en[_qp] - _u[_qp]);
  // _d_n_gamma_d_u = -1. * (1. - _a) * _user_se_coeff * _ion_flux * _normals[_qp] /
...
SakiyamaSecondaryElectronBC::computeQpOffDiagJacobian(unsigned int jvar)
{
  if (jvar == _potential_id)
  {
    ...
    // _n_gamma = (1. - _a) * _user_se_coeff * _ion_flux * _normals[_qp] / (_muem[_qp] *
    // -_grad_potential[_qp] * _r_units * _normals[_qp] + std::numeric_limits<double>::epsilon());
    // _d_n_gamma_d_potential = (1. - _a) * _user_se_coeff / _muem[_qp] * (_d_ion_flux_d_potential *
    ...
    return /*_test[_i][_qp] * _r_units * (1. - _r) / (1. + _r) * (- 0.5 * _v_thermal *
        ...
        -_test[_i][_qp] * _r_units * 2. / (1. + _r) * (1. - _a) * _user_se_coeff *
  ...
  else if (jvar == _mean_en_id)
  {
    ...
    // -_grad_potential[_qp] * _r_units * _normals[_qp] + std::numeric_limits<double>::epsilon());
    _actual_mean_en = std::exp(_mean_en[_qp] - _u[_qp]);
    // _d_n_gamma_d_mean_en = -1. * (1. - _a) * _user_se_coeff * _ion_flux * _normals[_qp] /
    // (std::pow(_muem[_qp] * -_grad_potential[_qp] * _r_units * _normals[_qp], 2.) +
    // std::numeric_limits<double>::epsilon()) * -_grad_potential[_qp] * _r_units * _normals[_qp] *
    // _d_muem_d_actual_mean_en[_qp] * _actual_mean_en * _phi[_j][_qp];
  ...
  else if (jvar == _ip_id)
  {
    ...
    // _d_n_gamma_d_ip = (1. - _a) * _user_se_coeff * _d_ion_flux_d_ip * _normals[_qp] / (_muem[_qp]
    // * -_grad_potential[_qp] * _r_units * _normals[_qp] + std::numeric_limits<double>::epsilon());
