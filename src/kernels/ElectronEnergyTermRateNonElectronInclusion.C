#include "ElectronEnergyTermRateNonElectronInclusion.h"

registerMooseObject("ZapdosApp", ElectronEnergyTermRateNonElectronInclusion);

template <>
InputParameters
validParams<ElectronEnergyTermRateNonElectronInclusion>()
{
  InputParameters params = validParams<Kernel>();
  params.addRequiredCoupledVar("em", "The electron density.");
  params.addCoupledVar("v", "The first reactant species.");
  params.addCoupledVar("w", "The second reactant species.");
...
ElectronEnergyTermRateNonElectronInclusion::ElectronEnergyTermRateNonElectronInclusion(
...
Real
ElectronEnergyTermRateNonElectronInclusion::computeQpResidual()
{
  // if (_elastic)
  //   _energy_change = _elastic_energy[_qp];
  // else
  ...
  //if (isCoupled("v"))
    return -_test[_i][_qp] * _rate_coefficient[_qp] * std::exp(_v[_qp]) * std::exp(_w[_qp]) * _energy_change;
  //else
  //  return -_test[_i][_qp] * _rate_coefficient[_qp] * _n_gas[_qp] * std::exp(_em[_qp]) * _energy_change;
}

Real
ElectronEnergyTermRateNonElectronInclusion::computeQpJacobian()
{
  // if (_elastic)
  //   _energy_change = _elastic_energy[_qp];
  // else
  ...
  //if (isCoupled("v"))
    return -_test[_i][_qp] * d_iz_d_mean_en * std::exp(_v[_qp]) * std::exp(_w[_qp]) * _energy_change;
  //else
  //  return -_test[_i][_qp] * d_iz_d_mean_en * _n_gas[_qp] * std::exp(_em[_qp]) * _energy_change;
}

Real
ElectronEnergyTermRateNonElectronInclusion::computeQpOffDiagJacobian(unsigned int jvar)
{
  // if (_elastic)
  //   _energy_change = _elastic_energy[_qp];
  // else
  ...
  //{
    ...
    if (_v_id == _v_id)
    {
      if (jvar == _v_id)
      {
        return -_test[_i][_qp] * 2 * std::exp(_v[_qp]) * std::exp(_w[_qp]) * _phi[_j][_qp] * _energy_change;
      }
      else
      {
        return 0.0;
