...
#include "EEDFReactionForShootMethod.h"

registerMooseObject("ZapdosApp", EEDFReactionForShootMethod);

InputParameters
EEDFReactionForShootMethod::validParams()
{
...
EEDFReactionForShootMethod::EEDFReactionForShootMethod(const InputParameters & parameters)
  : ADKernel(parameters),
...
ADReal
EEDFReactionForShootMethod::computeQpResidual()
{
  return -_test[_i][_qp] * _stoichiometric_coeff * _reaction_coeff[_qp] *
         _u[_qp] * _electron[_qp];
}
