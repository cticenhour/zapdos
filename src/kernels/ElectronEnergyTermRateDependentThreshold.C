#include "ElectronEnergyTermRateDependentThreshold.h"

registerMooseObject("ZapdosApp", ElectronEnergyTermRateDependentThreshold);

template <>
InputParameters
validParams<ElectronEnergyTermRateDependentThreshold>()
{
...
ElectronEnergyTermRateDependentThreshold::ElectronEnergyTermRateDependentThreshold(
...
Real
ElectronEnergyTermRateDependentThreshold::computeQpResidual()
{
  // if (_elastic)
  //   _energy_change = _elastic_energy[_qp];
  // else
  ...
  Real _energy_change = std::exp(_u[_qp] - _em[_qp]);
...
Real
ElectronEnergyTermRateDependentThreshold::computeQpJacobian()
{
  // if (_elastic)
  //   _energy_change = _elastic_energy[_qp];
  // else
  ...
  Real _energy_change = std::exp(_u[_qp] - _em[_qp]);
  Real d_energy_change_d_mean_en = std::exp(_u[_qp] - _em[_qp]) * _phi[_j][_qp];

  if (isCoupled("v"))
    return -_test[_i][_qp] * d_iz_d_mean_en * std::exp(_v[_qp]) * std::exp(_em[_qp]) * _energy_change +
          -_test[_i][_qp] * _rate_coefficient[_qp] * std::exp(_v[_qp]) * std::exp(_em[_qp]) * d_energy_change_d_mean_en;
  else
    return -_test[_i][_qp] * d_iz_d_mean_en * _n_gas[_qp] * std::exp(_em[_qp]) * _energy_change +
           -_test[_i][_qp] * _rate_coefficient[_qp] * _n_gas[_qp] * std::exp(_em[_qp]) * d_energy_change_d_mean_en;
}

Real
ElectronEnergyTermRateDependentThreshold::computeQpOffDiagJacobian(unsigned int jvar)
{
  // if (_elastic)
  //   _energy_change = _elastic_energy[_qp];
  // else
  ...
  Real _energy_change = std::exp(_u[_qp] - _em[_qp]);
  Real d_energy_change_d_em = std::exp(_u[_qp] - _em[_qp]) * -_phi[_j][_qp];

  if (isCoupled("v"))
  {
    if (jvar == _em_id)
    {
      return -_test[_i][_qp] * _rate_coefficient[_qp] * std::exp(_em[_qp]) * std::exp(_v[_qp]) * _phi[_j][_qp] * d_energy_change_d_em;
    }
    if (jvar == _v_id)
    {
  ...
  {
    if (jvar == _em_id)
      return -_test[_i][_qp] * _rate_coefficient[_qp] * std::exp(_em[_qp]) * _n_gas[_qp] * _phi[_j][_qp] * d_energy_change_d_em;
    else
      return 0.0;
  }
